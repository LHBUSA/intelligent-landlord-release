...
function NetworkLink({ site }: { site: typeof NETWORK[0] }) {
  ...
  return (
    <a href={site.href} target="_blank" rel="noopener noreferrer" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, textDecoration: 'none', gap: 8, padding: '10px 12px', border: `1px solid ${hovered ? 'rgba(45,212,191,0.35)' : 'rgba(255,255,255,0.05)'}`, background: hovered ? 'rgba(45,212,191,0.05)' : 'transparent', transition: 'all 0.2s ease', transform: hovered ? 'translateX(4px)' : 'translateX(0)' }}>
      <div>
