  ...
  return (
    <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', padding: 'clamp(40px,6vw,72px) var(--page-pad)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 1 }}>
        {STATES.map(s => (
          <Link key={s.abbr} href={`/legal/states/${s.abbr.toLowerCase()}`}
            style={{ display: 'block', background: 'var(--bg2)', border: '1px solid var(--border2)', padding: '16px 18px', transition: 'border-color 0.2s' }}
