...
export default function CategoryPage() {
  ...
  return (
    <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', padding: 'clamp(40px,6vw,72px) var(--page-pad)' }}>
      ...
      <h1 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 'clamp(2rem,4vw,3.2rem)', fontWeight: 600, marginBottom: 14 }}>Rental Market</h1>
      <p style={{ color: 'var(--muted)', maxWidth: '60ch', lineHeight: 1.65, marginBottom: 48 }}>Cap rates, vacancy trends, rent growth data, and market intelligence for rental property investors.</p>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 1 }}>
        {articles.map(a => (
          <Link key={a.slug} href={`/market/${a.slug}`} style={{ display: 'block', background: 'var(--bg2)', border: '1px solid var(--border2)', padding: 28 }}>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--gold)', marginBottom: 10 }}>{a.pill} · {a.readTime}</div>
